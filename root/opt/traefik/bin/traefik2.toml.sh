...
TRAEFIK_ACCESS_BUFFER=${TRAEFIK_ACCESS_BUFFER:-"0"}
...
TRAEFIK_MIDDLEWARE_BUFFER_MAXREQBODY=${TRAEFIK_MIDDLEWARE_BUFFER_MAXREQBODY:-""}
TRAEFIK_MIDDLEWARE_BUFFER_MEMREQBODY=${TRAEFIK_MIDDLEWARE_BUFFER_MEMREQBODY:-""}
TRAEFIK_MIDDLEWARE_BUFFER_MAXRESPBODY=${TRAEFIK_MIDDLEWARE_BUFFER_MAXRESPBODY:-""}
TRAEFIK_MIDDLEWARE_BUFFER_MEMRESPBODY=${TRAEFIK_MIDDLEWARE_BUFFER_MEMRESPBODY:-""}
TRAEFIK_MIDDLEWARE_BUFFER_RETRYEXPESS=${TRAEFIK_MIDDLEWARE_BUFFER_RETRYEXPESS:-""}
TRAEFIK_MIDDLEWARE_CIRCUITBREAKER=${TRAEFIK_MIDDLEWARE_CIRCUITBREAKER:-""}
TRAEFIK_MIDDLEWARE_COMPRESS_ENABLE=${TRAEFIK_MIDDLEWARE_COMPRESS_ENABLE:-"false"}
TRAEFIK_MIDDLEWARE_OPTS=${TRAEFIK_MIDDLEWARE_OPTS:-""}
...
#Global Middlewares#
####################
TRAEFIK_MIDDLEWARE_OPTS="\
[http.middlewares]
"
if [ "${TRAEFIK_MIDDLEWARE_COMPRESS_ENABLE}" == "true" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
  [http.middlewares.global-compress.compress]
"
fi
if [ "${TRAEFIK_MIDDLEWARE_CIRCUITBREAKER}" != "" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
  [http.middlewares.global-circuitbreaker.circuitBreaker]
    expression = \"${TRAEFIK_MIDDLEWARE_CIRCUITBREAKER}\"
"
fi
if [ "${TRAEFIK_MIDDLEWARE_BUFFER_MAXREQBODY}" != "" ] || [ "${TRAEFIK_MIDDLEWARE_BUFFER_MEMREQBODY}" != "" ] || [ "${TRAEFIK_MIDDLEWARE_BUFFER_MAXRESPBODY}" != "" ] || [ "${TRAEFIK_MIDDLEWARE_BUFFER_MEMRESPBODY}" != "" ] || [ "${TRAEFIK_MIDDLEWARE_BUFFER_RETRYEXPESS}" != "" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
  [http.middlewares.global-buffering.buffering]
"
  if [ "${TRAEFIK_MIDDLEWARE_BUFFER_MAXREQBODY}" != "" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
    maxRequestBodyBytes = ${TRAEFIK_MIDDLEWARE_BUFFER_MAXREQBODY}
"
  fi
  if [ "${TRAEFIK_MIDDLEWARE_BUFFER_MEMREQBODY}" != "" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
    memRequestBodyBytes = ${TRAEFIK_MIDDLEWARE_BUFFER_MEMREQBODY}
"
  fi
  if [ "${TRAEFIK_MIDDLEWARE_BUFFER_MAXRESPBODY}" != "" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
    maxResponseBodyBytes = ${TRAEFIK_MIDDLEWARE_BUFFER_MAXRESPBODY}
"
  fi
  if [ "${TRAEFIK_MIDDLEWARE_BUFFER_MEMRESPBODY}" != "" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
    memResponseBodyBytes = ${TRAEFIK_MIDDLEWARE_BUFFER_MEMRESPBODY}
"
  fi
  if [ "${TRAEFIK_MIDDLEWARE_BUFFER_RETRYEXPESS}" != "" ]; then
    TRAEFIK_MIDDLEWARE_OPTS=$TRAEFIK_MIDDLEWARE_OPTS"\
    retryExpression = \"${TRAEFIK_MIDDLEWARE_BUFFER_RETRYEXPESS}\"
"
  fi
fi

cat << EOF > ${SERVICE_HOME}/dynamic/middlewares.toml
${TRAEFIK_MIDDLEWARE_OPTS}
EOF
...
[accessLog]
  ...
  bufferingSize = ${TRAEFIK_ACCESS_BUFFER}
